...
class OutletTest extends TestCase
{
    ...
    /** @test */
    public function an_outlet_has_observations_link_attribute()
    {
        $outlet = factory(Outlet::class)->create();

        $title = __('app.show_detail_title', [
            'observations' => $outlet->observations, 'type' => __('outlet.outlet'),
        ]);
        $link = '<a href="'.route('outlets.show', $outlet).'"';
        $link .= ' title="'.$title.'">';
        $link .= $outlet->observations;
        $link .= '</a>';

        $this->assertEquals($link, $outlet->observations_link);
    }
    ...
    public function an_outlet_has_map_popup_content_attribute()
    {
        ...
        $mapPopupContent = '';
        $mapPopupContent .= '<div class="my-2"><strong>'.__('outlet.observations').':</strong><br>'.$outlet->observations_link.'</div>';
